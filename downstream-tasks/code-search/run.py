...
def evaluate(args, model, tokenizer,file_name,eval_when_training=False):
    ...
    for batch in query_dataloader:  
        nl_inputs = batch[1].to(args.device)  # batch[1]是指（代码-描述对）中的描述部分
        with torch.no_grad():
            nl_vec = model(nl_inputs=nl_inputs)
    ...
    scores = np.matmul(nl_vecs,code_vecs.T)  # 通过矩阵乘法得到查询对应的每一个代码的相似度得分
    
    sort_ids = np.argsort(scores, axis=-1, kind='quicksort', order=None)[:,::-1]  # 按照相似度降序排序，返回的为索引列表
    ...
    ranks = []
    for url, sort_id in zip(nl_urls,sort_ids):  # 计算MRR
        rank = 0
        find = False
